...
const DEFAULT_WEIGHT_REGEX = /([+-]?\d+\.?\d*)\s*(kg)?/i;
...
export class SerialPortManager extends EventEmitter {
  ...
  private handleData(line: string): void {
    ...
    // Accumulate buffer in case of partial data
    this.buffer += trimmed;

    const match = this.buffer.match(this.weightRegex);
    if (match) {
      ...
      // If buffer gets too large without a match, trim it
